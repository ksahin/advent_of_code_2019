class Tree(object):
    ...
    def find_common_ancestor(self, node_a, node_b):
        ancestor_a = []
        ancestor_b = []
        current_node = node_a
        while current_node.parent is not None:
            ancestor_a.append(current_node)
            current_node = current_node.parent
        current_node = node_b
        while current_node.parent is not None:
            ancestor_b.append(current_node)
            current_node = current_node.parent
        for ancestor in ancestor_a:
            if ancestor in ancestor_b:
                return ancestor
...
san_orbit = root.findObjectByName('SAN').parent
you_orbit = root.findObjectByName('YOU').parent
lowest_common_ancestor = root.find_common_ancestor(san_orbit, you_orbit)

print(lowest_common_ancestor.get_orbit_number(san_orbit.name, 0) + lowest_common_ancestor.get_orbit_number(you_orbit.name, 0))